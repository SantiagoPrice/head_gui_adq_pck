#! /usr/bin/env python3
'''
This program works for: with two IMUs attached on the same device, with different initial poses (under certain conditions), keep measuring the relative roll(around x axis) between two IMUs no matter the pose changes after initial moment.
...
the axis of the openzen LPMS-B2 IMUs: https://lp-research.atlassian.net/wiki/spaces/LKB/pages/1100611599/LPMS+User+Manual
'''
import time
import math
import os
...
def self_relative():
    ...
    relative_imu2_e = (Ro.from_quat([relative_imu2_q.x, relative_imu2_q.y, relative_imu2_q.z, relative_imu2_q.w])).as_euler("xyz", degrees=True)
...
def imu_measure_node():
    ...
    rospy.Subscriber("/imu3/data", Imu, callback_imu2, queue_size=1) # imu2
