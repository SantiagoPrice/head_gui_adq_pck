...
offset_imu1_pose = np.quaternion(1.,0.,0.,0.) 
offset_imu2_pose = np.quaternion(1.,0.,0.,0.) 


angles = [ "pitch_sp" , "roll_sp" , "yaw_sp" , "pitch_hd" , "roll_hd" , "yaw_hd" , "pitch_ref" , "roll_ref" , "yaw_ref" ]
...
def callback_offset(flag):
    global offset_imu1_pose , offset_imu2_pose
    a,b,aux= self_relative() 
    offset_imu1_pose= a*offset_imu1_pose
    offset_imu2_pose = b
    print(offset_imu1_pose)
...
def self_relative():
    global imu1_q, imu2_q, initial_imu1, initial_imu2 , offset_imu1_pose ,offset_imu2_pose   #, relative_imu1_q, relative_imu2_q
    # relative_imu1_q  = imu1_q / initial_imu1
    # relative_imu1_q  = imu1_q * initial_imu1.inverse()
    relative_imu1_q  =  1/initial_imu1 * imu1_q * 1 / offset_imu1_pose
    relative_imu2_q  = 1/initial_imu2 * imu2_q
    imu2_relative_imu1_q = 1/(relative_imu1_q*offset_imu1_pose) *  (relative_imu2_q * 1/ offset_imu2_pose) * offset_imu1_pose
...
def visualize_posit(q,q2):
    ...
    head_state.position[3:6]= yawPitchRoll(q2, ls = True)
    head_state.position[6:]= ref_ypr
    return head_state
...
def imu_measure_node():
    ...
    while not rospy.is_shutdown():
        ...
        #print(1/offset_imu1_pose * imu2_relative_imu1_q )
        #imu2_relative_imu1_q= imu2_relative_imu1_q  / offset_imu1_pose 
        #displayIMUs(relative_imu1_q, relative_imu2_q, imu2_relative_imu1_q)

        imu1_realtive = visulaize_imu(1/relative_imu1_q)
        ...
        head_angle = visualize_posit(relative_imu1_q,imu2_relative_imu1_q)
