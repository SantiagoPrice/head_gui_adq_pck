#! /usr/bin/env python3
'''
This program is for measuring human head pose relative to body, focusing on the z axis and y axis.
The two imus don't have to be under the same pysical poses before starting this program,
...
def initial_pose():
    ...
    # initial_imu1_e = (Ro.from_quat([initial_imu1.x, initial_imu1.y, initial_imu1.z, initial_imu1.w])).as_euler("zxy")
    initial_imu1_e = tf.transformations.euler_from_quaternion((initial_imu1.x, initial_imu1.y, initial_imu1.z, initial_imu1.w))
    # print(initial_imu1_e)
    initial_imu1_e = list(initial_imu1_e)
    initial_imu1_e[0] = 0.
    # initial_imu2_e = (Ro.from_quat([initial_imu2.x, initial_imu2.y, initial_imu2.z, initial_imu2.w])).as_euler("zxy")
    initial_imu2_e = tf.transformations.euler_from_quaternion((initial_imu2.x, initial_imu2.y, initial_imu2.z, initial_imu2.w))
    initial_imu2_e = list(initial_imu2_e)
    initial_imu2_e[0] = 0.

    # initial_imu1 = Ro.from_euler('zyx', initial_imu1, degrees=True).as_quat()
    # initial_imu2 = Ro.from_euler('zyx', initial_imu2, degrees=True).as_quat()

    # initial_imu1 = tf.transformations.quaternion_from_euler(initial_imu1_e[0], initial_imu1_e[1], initial_imu1_e[2])
    # initial_imu2 = tf.transformations.quaternion_from_euler(initial_imu1_e[0], initial_imu1_e[1], initial_imu1_e[2])

    initial_imu1 = quaternion.from_euler_angles(initial_imu1_e)
    # initial_imu1 = quaternion.as_quat_array(initial_imu1_quaternion)

    initial_imu2 = quaternion.from_euler_angles(initial_imu2_e)
    # initial_imu2 = quaternion.as_quat_array(initial_imu2_quaternion)
...
def self_relative():
    ...
    # relative_imu1_e = (Ro.from_quat([relative_imu1_q.x, relative_imu1_q.y, relative_imu1_q.z, relative_imu1_q.w])).as_euler("xyz", degrees=True)
    # relative_imu2_e = (Ro.from_quat([relative_imu2_q.x, relative_imu2_q.y, relative_imu2_q.z, relative_imu2_q.w])).as_euler("yxz", degrees=True)
    # relative_imu2_e = tf.transformations.euler_from_quaternion((relative_imu2_q.x, relative_imu2_q.y, relative_imu2_q.z, relative_imu2_q.w), "syxz")
    # relative_imu2_e = [i*180/np.pi for i in relative_imu2_e]
    # print("relative_imu2_e", np.round(relative_imu2_e, decimals = 1))

    relative_imu1_e = tf.transformations.euler_from_quaternion((relative_imu1_q.x, relative_imu1_q.y, relative_imu1_q.z, relative_imu1_q.w))
    relative_imu1_e = list(relative_imu1_e)
    relative_imu1_e[0] = 0.

    relative_imu1_q = quaternion.from_euler_angles(relative_imu1_e)
    ...
    # imu2_relative_imu1_e = (Ro.from_quat([imu2_relative_imu1_q.x, imu2_relative_imu1_q.y, imu2_relative_imu1_q.z, imu2_relative_imu1_q.w])).as_euler("xyz", degrees=True)
    imu2_relative_imu1_e = tf.transformations.euler_from_quaternion((imu2_relative_imu1_q.x, imu2_relative_imu1_q.y, imu2_relative_imu1_q.z, imu2_relative_imu1_q.w), "sxyz")
