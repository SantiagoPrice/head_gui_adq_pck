...
def self_relative():
    ...
    euler = tf.transformations.euler_from_quaternion((imu1_q.x, imu1_q.y, imu1_q.z, imu1_q.w))
    euler[1] = 0
    euler[2] = 0
    relative_imu1_q = tf.transformations.quaternion_from_euler(euler.x, euler.y, euler.z)
    relative_imu1_e = [i/math.pi*180 for i in euler]
    relative_imu1_e = np.round(relative_imu1_e, decimals = 1)
    relative_imu1_q = np.quaternion(relative_imu1_q.w, relative_imu1_q.x, relative_imu1_q.y, relative_imu1_q.z)


    relative_imu2_q  = 1/initial_imu2 * imu2_q
    euler = tf.transformations.euler_from_quaternion((imu2_q.x, imu2_q.y, imu2_q.z, imu2_q.w))
    euler.x = 0
    euler.y = 0
    relative_imu2_q = tf.transformations.quaternion_from_euler(euler.x, euler.y, euler.z)
    relative_imu2_e = [i/math.pi*180 for i in euler]
    relative_imu2_e = np.round(relative_imu2_e, decimals = 1)
    relative_imu2_q = np.quaternion(relative_imu2_q.w, relative_imu2_q.x, relative_imu2_q.y, relative_imu2_q.z)
