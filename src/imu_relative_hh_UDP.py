...
def callback_offset(flag):
    global   offset_imu_pose , imu1_q , initial_imu1
    offset_imu_pose= 1/imu1_q * initial_imu1
...
def self_relative():
    ...
    relative_imu1_q  = 1/imu1_q * initial_imu1 * 1/ offset_imu_pose  
    #print(f"raw:{imu1_q}")
    #print(f"init:{initial_imu1}")
    #print(f"off:{offset_imu_pose  }")
...
def imu_measure_node():
    global self_check_flag, initializ_flag, num_msg_recieved, relative_imu_pose , ERR_TOL , ref_flag, audiofile, relative_imu1_q#, relative_imu1_q, relative_imu2_q
    ########### Starting ROS Node ###########
    ...
    rospy.Subscriber("/adq/imu_UDP/data", Imu, callback_UDP, queue_size=1) # imu1
    ...
    while not rospy.is_shutdown():
        ...
        if initializ_flag == 1:
            initial_pose()
            initializ_flag = 0
        relative_imu1_q= self_relative()   
        head_angle = visualize_posit(relative_imu1_q )
