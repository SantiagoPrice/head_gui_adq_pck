...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        checkboxes = []
        checkbox_names = [f'{int((i)*45/3)}' for i in range(4)]
        ...
        reset_button = QPushButton('Reset Q and F')
    ...
    def IMU_callback(self, q):
        ...
        if self.record["cond"]:# and self.save_checkbox.isChecked():
                ...
                F_txt=[str(fc) for fc in [FXYZ.x,FXYZ.y,FXYZ.z]]
                with open(filename, 'a') as file:
                    #file.write(','.join(str(ang) for ang in list(rpy)+F_txt)+'\n')
                    file.write(','.join(str(ang) for ang in q_rv_lst+F_txt)+'\n')
